...
public class GDM4 implements PlugInFilter {
	...
	final static String[] choices = { "Wischen", "Weiche Blende", "Overlay", "Schieb-Blende", "Chroma Key", "Extra",
			"Probeklausur 10.1", "Probeklausur 10.2" };
	...
	public void run(ImageProcessor B_ip) {
		...
		if (s.equals("Extra"))
			methode = 6;
		if (s.equals("Probeklausur 10.1"))
			methode = 7;
		if (s.equals("Probeklausur 10.2"))
			methode = 8;
			...
			for (int y = 0; y < height; y++) {
				for (int x = 0; x < width; x++, pos++) {
					...
					if (methode == 4) // Schieb-Blende
					{
						...
						// Je weiter rechts und je weiter links er ist, desto eher wird die
						// else-Schleife ausgeführt
						{
							pixels_Erg[pos] = pixels_A[pos - temp];
							// Das erste Bild wird nach rechts geschoben, der Pixel im neuen Bild ist im
							// BildA
							// weiter links gewesen
						} else {
							pixels_Erg[pos] = pixels_B[(int) pos + (width - temp)];
							// je weiter der Progress vorangeschritten ist, desto weiter wandern die Pixel
							// nach rechts
						}
					}
					...
					if (methode == 6) // Extra
					{
						...
						if ((x >= square1Edge1AtWeight && y >= square1Edge1AtHeight && x <= square1Edge2AtWeight
								&& y <= square1Edge2AtHeight)
								|| (x >= square2Edge1AtWeight && y >= square2Edge1AtHeight && x <= square2Edge2AtWeight
										&& y <= square2Edge2AtHeight)) {
							r = rB;
							g = gB;
							b = bB;
						} else {
