 ...
 */
public class ImageEditorYUV implements PlugIn {
    ...
    public static void main(String args[]) {
        IJ.open("E:\\workspace\\BasisVisuals\\Files\\orchid.jpg");
        ImageEditorYUV pw = new ImageEditorYUV();
        ...
        private void changePixelValues(ImageProcessor ip) {
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    ...
                    YUV yuv = inRGB.transformToYUV();

                    yuv = yuv.changeBrightness(brightness);
                    yuv = yuv.changeContrast(contrast);
                    yuv = yuv.changeSaturation(saturation);
                    yuv = yuv.changeHue(hue);


                    if(x == 128 && y == 128) {
                        System.out.println("Y: " + yuv.getY());
                        System.out.println("U: " + yuv.getU());
                        System.out.println("V: " + yuv.getV() + "\n");
                    }

                    // Valuecheck in class RGB
                    RGB outRGB = yuv.transformToRGB();
